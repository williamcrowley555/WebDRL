<?php
    ...
    if($data["status"]==1){
        if ($checkQuyen->checkQuyen_CVHT_Khoa_CTSV_Admin($data["user_data"]->aud)) {
            ...
            $item = new LopMoNhapDiemHe4($db); //new Lop object
            ...
            if ($data != null){
                ...
                if($item->createLopMoNhapDiemHe4()){
                    http_response_code(200);
                    echo json_encode(
                        array("message" => "lopmonhapdiemhe4 tạo thành công!")
                    );
                } else{
                    http_response_code(500);
                    echo json_encode(
                        array("message" => "lopmonhapdiemhe4 tạo KHÔNG thành công!")
                    );
                }
            }else{
