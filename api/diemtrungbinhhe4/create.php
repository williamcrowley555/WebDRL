    ...
    if($data["status"]==1){
        if ($checkQuyen->checkQuyen_Khoa_CTSV($data["user_data"]->aud)) {
            ...
            if ($data != null){
                ...
                if($item->createDiemTrungBinhHe4()){
                    http_response_code(200);
                    echo json_encode(
                        array("message" => "diemtrungbinhhe4 tạo thành công!")
                    );
                } else {
                    http_response_code(404);
                    echo json_encode(
                        array("message" => "Điểm trung bình học kỳ này đã tồn tại!")
                    );
                } 
                // else {
                //     http_response_code(500);
                //     echo json_encode(
                //         array("message" => "diemtrungbinhhe4 tạo KHÔNG thành công!")
                //     );
                // } 
            }else{
                http_response_code(404);
