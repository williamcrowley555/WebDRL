<?php
    ...
    $fileName = "danh_sach_lop_" . date('d-m-Y');
    ...
    if(isset($_POST["btn_export_to_excel"])) {
        ...
        if($tableContent != null && count($tableContent) > 0) {
            ...
            for ($i = 0; $i < count($tableContent); $i++) {
                ...
                $sheet->setCellValue($alphabet[1] . $rowCount, $tableContent[$i]['maLop']);
                $sheet->setCellValue($alphabet[2] . $rowCount, $tableContent[$i]['tenLop']);
                $sheet->setCellValue($alphabet[3] . $rowCount, $tableContent[$i]['maKhoa']);
                $sheet->setCellValue($alphabet[4] . $rowCount, $tableContent[$i]['maCoVanHocTap']);
                $sheet->setCellValue($alphabet[5] . $rowCount, $tableContent[$i]['maKhoaHoc']);
