<?php
    ...
    $fileName = "danh_sach_khoa_" . date('d-m-Y');
    ...
    if(isset($_POST["btn_export_to_excel"])) {
        ...
        if($tableContent != null && count($tableContent) > 0) {
            ...
            for ($i = 0; $i < count($tableContent); $i++) {
                ...
                $sheet->setCellValue($alphabet[1] . $rowCount, $tableContent[$i]['maKhoa']);
                $sheet->setCellValue($alphabet[2] . $rowCount, $tableContent[$i]['tenKhoa']);
                $sheet->setCellValue($alphabet[3] . $rowCount, $tableContent[$i]['taiKhoanKhoa']);
