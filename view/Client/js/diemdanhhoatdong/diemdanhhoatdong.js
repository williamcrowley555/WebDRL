    ...
    if (GET_maHoatDong.trim() != ''){
      ...
      $.ajax({
        ...
        success: function (result_HoatDongDanhGia) {
            ...
            var thoiGianBatDauHoatDong = new Date(result_HoatDongDanhGia.thoiGianBatDauHoatDong);
            var thoiGianKetThucHoatDong = new Date(result_HoatDongDanhGia.thoiGianKetThucHoatDong);
          
            //lấy ngày hiện tại
            var today = new Date();

            if (today.getTime() >= thoiGianBatDauHoatDong.getTime() 
...
function getThongTinHoatDong(){
    ...
    $.ajax({
        ...
        success: function (result_DanhGia) {
            ...
            var thoiGianBatDauHoatDong = new Date(result_DanhGia.thoiGianBatDauHoatDong);
            var thoiGianKetThucHoatDong = new Date(result_DanhGia.thoiGianKetThucHoatDong);
            var maHocKyDanhGia = result_DanhGia.maHocKyDanhGia;
            var thoiGianBatDauDiemDanh = new Date(result_DanhGia.thoiGianBatDauDiemDanh);
                    ...
                    success: function (result_Khoa) {
                        ...
                        $("#part_thongTinHoatDong").append("\
                            <div class='list-group-item list-group-item-action' >\
                              <span style='font-weight: bold;' >Mã hoạt động: </span><span id='input_MaHoatDong' >"+ GET_maHoatDong +"</span> \
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                              <span style='font-weight: bold;'>Tên hoạt động: </span>"+ tenHoatDong +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                                  <span style='font-weight: bold;'>Khoa tổ chức: </span>"+ tenKhoa +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                                <span style='font-weight: bold;'>Học kỳ áp dụng: </span>"+ hocKyXet + " - Năm học: "+ namHocXet  +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                                <span style='font-weight: bold;'>Điểm nhận được: </span>"+ diemNhanDuoc +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                                <span style='font-weight: bold;'>Mục được cộng điểm: </span>"+ tenTieuChi +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                                <span style='font-weight: bold;'>Địa điểm diễn ra hoạt động: </span>"+ diaDiemDienRaHoatDong +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                                <span style='font-weight: bold;'>Thời gian bắt đầu: </span>"+ thoiGianBatDauHoatDong.toLocaleString() +"\
                            </div>\
                            <div class='list-group-item list-group-item-action' >\
                        ...
                        if (isNaN(Date.parse(thoiGianBatDauDiemDanh))){
                            $("#text_thoiGianDiemDanh").text("Ban tổ chức chưa mở thời gian điểm danh!"); 
                            //$("#btn_DiemDanh").remove();
                        }else{
                          //Thêm 5 phút từ lúc nhấn nút bắt đầu điểm danh
                          var countDownDate = new Date(thoiGianBatDauDiemDanh.setMinutes(thoiGianBatDauDiemDanh.getMinutes() + 5)).getTime();

                          $("#part_DiemDanh").append("<button type='button' class='btn btn-warning' style='width: auto;color: white;font-size: x-large;' id='btn_DiemDanh' onclick='return DiemDanh()' >\
                            Điểm danh</button>");
...
function setCountDownDiemDanh(countDownDate) {
  var myfunc = setInterval(function() {
    ...
    document.getElementById("text_thoiGianDiemDanh").innerHTML = "Thời gian điểm danh còn lại:"
    document.getElementById("text_Timer").innerHTML = ('0' + minutes).slice(-2) + ":"  + ('0' + seconds).slice(-2)
    ...
    if (timeleft < 0) {
        ...
        document.getElementById("text_thoiGianDiemDanh").innerHTML = "Đã hết thời gian điểm danh!";
        $("#btn_DiemDanh").remove();
    }
    }, 1000);
